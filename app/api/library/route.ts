...
// Every handler is wrapped so an unexpected throw (bad cookie, network blip,
// oversized headers) logs the real cause and returns clean JSON, never an
// opaque 500.
...
export async function GET() {
  try {
    ...
    if (error) {
      console.error("threem: library GET error", error);
    ...
    console.error("threem: library GET threw", err);
    return NextResponse.json({ error: "request failed" }, { status: 500 });
  }
}
...
export async function POST(req: NextRequest) {
  try {
    ...
    if (error) {
      console.error("threem: library POST error", error);
      return NextResponse.json({ error: "request failed" }, { status: 500 });
    }
    return NextResponse.json({ ok: true });
  } catch (err) {
    console.error("threem: library POST threw", err);
    return NextResponse.json({ error: "request failed" }, { status: 500 });
  }
}
...
export async function DELETE(req: NextRequest) {
  try {
    ...
    const { error } = await supabase
      ...
      .eq("game_id", game_id);
    if (error) {
      console.error("threem: library DELETE error", error);
      return NextResponse.json({ error: "request failed" }, { status: 500 });
    }
    return NextResponse.json({ ok: true });
  } catch (err) {
    console.error("threem: library DELETE threw", err);
    return NextResponse.json({ error: "request failed" }, { status: 500 });
  }
}
